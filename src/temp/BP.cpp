		...
		if(numNodesToProcess > 50)
		{
			...
			[&](const tbb::blocked_range<int>& r)
			{
				for (int i=r.begin();i!=r.end();++i)
					...
					//Test_ParallelBPFromNode_1DPartitioning_UpdateNode(pGraph, visitedNodes, queue, true);
			}
		);
		}
		else
...
void ParallelBPFromNode_1DPartitioning_Hybrid(TPt<TNodeEDatNet<TFlt, TFlt>>& pGraph, int sourceNodeID)
{
	static tbb::spin_mutex sMutex;
	tbb::concurrent_hash_map<int, bool> visitedNodes;
	tbb::concurrent_queue<int> queue;
	...
	while(true)
	{
		int numNodesToProcess = queue.unsafe_size();
		// Parse level l
		if(numNodesToProcess > chunk)
		{
			#pragma omp parallel shared(pGraph,queue,visitedNodes,chunk) private(i)
			{
